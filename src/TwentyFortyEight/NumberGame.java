...
public class NumberGame implements NumberSlider {
     ...
     *                                  when the winning value is not power of two,
     *                                  or negative
     ********************************************************************/
                ...
                for (rowHelp = 0; rowHelp < board.length; rowHelp++) {
                    ...
                    if (this.wasChange(help, SlideDirection.RIGHT)) {
                        ...
                        for (h = 0; h < help.length; h++) {
                            if (help[h] != null) {
                                board[rowHelp][board[0].length - count] = help[h];
                                board[rowHelp][board[0].length - count].column = board[0].length - count;
                                count++;
                            }
                        }
                    } else {
            ...
            case DOWN:
                ...
                for (colHelp = 0; colHelp < board[0].length; colHelp++) {
                    ...
                    if (this.wasChange(help, SlideDirection.DOWN)) {
                        ...
                        for (h = 0; h < help.length; h++) {
                            if (help[h] != null) {
                                board[board.length - count][colHelp] = help[h];
                                board[board.length - count][colHelp].row = board.length - count;
                                count++;
                            }
                        }
                    } else {
    ...
    private Cell[] calcValues(Cell[] help) {
        ...
        for (h = 0; h < help.length; h++) {
            if (help[h] != null)
                if (h + 1 == help.length)
                    break;
            if (help[h].value * help[h + 1].value == Math.pow(help[h].value, 2)) {
    ...
    private boolean isGameOver() {
        ...
        if (this.getNonEmptyTiles().size() < board.length * board[0].length) {
            return false;
        }
